...
def ballot(voting_via_proxy=False):
    ...
    if voting_via_proxy:
        found = False
        for entry in data:
            if datetime.fromisoformat(entry["Day"]).date() == today:
                entry["Score"] += 1
                entry["Action"] += 1
                entry["Edge"] = (entry["Score"] / entry["Action"]) * 100
                storage.save_data(storage.SLIGHT_EDGE, data)
                found = True
                break
    ...
    footer = center(" Action Voted Successfully ", 1)
    print()
    print(footer)
...
def display(daily=0):
    ...
    trajectory = "Headed for success" if edge >= 50 else "Headed for failure"    
...
def edge_trendline(limit=7):
    print()    
    data = storage.load_data(storage.SLIGHT_EDGE)
    limit = min(limit, len(data))
    values = [round(entry["Edge"]) for entry in data[-limit:]]
    dates = [timetools.timestamp(e["Day"], 0, 1)[:2] for e in data[-limit:]]  # day only
    ...
    for y in range(100, -1, -5):
        ...
        for pos, v in enumerate(values):
            if v == 100:
                ...
                point = color("●", "red")
            filler = color("○", "gray")
            row += f"  {point} "  if y - 5 < v <= y else f"  {filler} "
        print(center(row))

    term_width = shutil.get_terminal_size((80, 
        20)).columns
    display_len = 28 if term_width > 46 else 23
    print(center("└" + "─" *  display_len, fixed=26))
    print(center("   " + "  ".join(dates), fixed=26))
    print()
