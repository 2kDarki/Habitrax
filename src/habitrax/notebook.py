...
def write():
    ...
    footer = center(" ✔ Page Saved Successfuly! ", 1)
    print(f"\n{footer}")
...
def read(page=None):
    ...
    if page:
        ...
        print(labels[0], page["Title"])
        print()
        print(page["Content"])
        print()
        print(labels[1], stamp)
        print()
        underline()
        return
...
def del_page():
    ...
    text = color(f"\nAre you sure you want to delete {page['Title']}? [y/n]: ", "red") 
    confirmation = input(text)[0].lower().strip()
    print()
    if confirmation != "y":
        footer = center(" ✔ Page Not Deleted ", 1)
        print(f"{footer}\n")
        return
    ...
    del notebook[choice]
    storage.save_data(storage.NOTEBOOK, notebook)
    footer = center(" ✔ Page Moved to Trashcan ", 1)
    print(f"{footer}")
...
def trashcan():
    ...
    storage.save_data(storage.NOTEBOOK, notebook)
    footer = center(" ✔ Page Restored Successfuly! ", 1)    
    print(f"\n{footer}")
