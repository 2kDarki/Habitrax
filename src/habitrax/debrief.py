from .toolbox import choose_from, timetools, no_data
from .toolbox import choose, list_options, underline
from .toolbox import warning, label, variables
from .toolbox import color, center, strip_ansi
...
def reflect():
    ...
    for n, item in enumerate(selected):
        x = len(f"Q{n+1}. ")
        print()
        print(wrap(f"Q{n+1}. {item['question']}", x))
 ...
 - kind=None: Shows service log style entries (used in soul_work.py)
    ...
    for e in entries:
        ...
        if kind == "reflections":
            ...
            subject = f"{labels[4]} {e['Entry']['Subject']}"
            service = f"{labels[5]} {e['Entry']['Service']}"
            print(wrap(subject, len(strip_ansi(
                labels[4])+1)))
            print(wrap(subject, len(strip_ansi(
                labels[5])+1)))
        else:
            for k, tag in {
                ...
                content = f"{color(f'{tag}:', 'cyan')} {e.get(k, '')}"
