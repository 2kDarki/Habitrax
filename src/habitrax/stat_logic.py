...
def rename():
    ...
    name = names[choice]
    new_name = input(color(f"\nEnter new name for {name.lower()}: "))
    rename_stat(name, new_name)
    footer = center(f" {name} Renamed Successfully ", 1)
    print(f"\n{footer}")
...
def combine():
    ...
    while True:
        try:
            ...
            if any(0 >= i or i > len(names) for i in indices):
                raise ValueError
            break
        except ValueError:
            pass
    ...
    for choice in choices:
        source_list.append(names[int(choice)-1])
        
    print()
    names = ["New"]
    if mapping["Combinations"]:
        names = [name for name in mapping["Combinations"]]
        names.append("New")
    ...
    if target_name == "New":
        print()
        target_name = input(labels[0]).title().strip()
        
    combine_stats(target_name, source_list)
    footer = center(f" {len(source_list)} Names Combined Successfully ", 1)
    print(f"\n{footer}")

def rename_stat(original, new_name):
    mapping = storage.load_map()
    mapping["Renames"][original] = new_name
    storage.save_data(storage.STAT_MAP, mapping)

def combine_stats(target_name, source_list):
    mapping = storage.load_map()
