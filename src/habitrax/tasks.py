...
def render_mission_card(mission, index):
    name = mission["Name"]
    objective = mission["Objective"]
    rate = mission["Progress"]
    bar = rated_bar(rate)
    rated = center(f'{bar[1]}')
    pad = center(f'{bar[1]}', get=True)[0]
    
    spacer(2)
    print(f"{color(f'Mission {index+1}:', 'lightcyan')} {name}")
    print(f"{color(f'Objective:', 'cyan')} {objective}\n")
    RATE = center("  REALIZATION ")
    print(f"{RATE}")
    print(" " * pad + bar[0])
    print(f"{rated}")
    spacer(2)
